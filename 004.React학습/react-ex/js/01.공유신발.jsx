...
function MainComponent() {
    ...
    // 가상돔에서 실제돔에 반영후 DOM에 구현할 JS코드는 어디에 넣어야하는가?
    // -> useEffect()
    // -> useLayoutEffect()
    console.log("1. 컴포넌트 그냥구역", document.querySelector(".img-box"));

    // 순수 useEffect
    // -> 매번 업에이트 시에도 실행 
    React.useEffect(() => {
        console.log("2. useEffect 순수");
    });

    // 빈 배열옵션 useEffect
    // -> 페이지 로딩후 단 한번만 실행
    React.useEffect(() => {
        console.log("3. useEffect 빈 배열옵션");
    },[]);

    // 의존성 배열옵션 useEffect
    // -> 페이지 로딩후 단 한번만 실행
    React.useEffect(() => {
        console.log("4-1. useEffect 의존성 배열옵션 test");
    },[test]);
    React.useEffect(() => {
        console.log("4-2. useEffect 의존성 배열옵션 dataNum");
    },[dataNum]);
    // 의존성이 다수일 경우 [] 배열형태의 옵션에 콤마로 연결하여 등록해준다.

    // 랜더링 후 화면출력전 상태
    React.useLayoutEffect(() => {
        console.log("5. useLayoutEffect")
        // 버튼을 display: none;
        // $('.btn-gong').hide()
    })
    // 의존성 테스트 함수
    ...
    return (
        <React.Fragment>
            ...
            <button onClick={testFn}>테스트 버튼</button>
            {/* 4. 상품리스트박스 */}
            <div className="gwrap">
