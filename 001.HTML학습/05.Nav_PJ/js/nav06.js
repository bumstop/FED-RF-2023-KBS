...
/**********************************************************
    ...
    -> 결과로 변경된 배열이 리턴됨.
    -> 새로 만들어진 배열데이터를 현재 자리에 그대로 출력하고자
    할때 배열 메서드를 join(구분자)를 이용하여 배열을 
    하나의 문자형 데이터로 만들어주면 편리하다.

    예) const aa = ['하하,'호호'];
    `<div>${aa.map((val) => `<h2>${val}</h2>`).join('')}</div>
    - 결과: `<div><h2>하하</h2><h2>호호</h2></div>`
    -> 구분자가 없는 태그로만 구성된 최종 데이터를 그자리에 출력함! 


    배열변수.map(배열데이터)
    const aa= ['신숙','상호','경미'];
    -> 배열데이터는 이름은데 뒤에 '씨'붙여서 보관하기로 
    
    aa = aa.map((val) => val + '씨'); {} 안쓸경우 return 생략한것
    -> 결과: ['신숙씨','상호씨','경미씨']
**********************************************************/
const aa = ["신숙", "상호", "경미"];
console.log(aa);
...
console.log(bb);
const cc = bb.map((val, idx) => `<div>${(idx + 1) + "." + val}</div>`);
console.log(cc);
